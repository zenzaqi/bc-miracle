...
AS
   SELECT `detail_jual_produk`.`dproduk_id` AS `dproduk_id`,
          ...
          if(ifnull(`detail_jual_produk`.`dproduk_diskon_jenis`, '-') = '',
             '-',
             ifnull(`detail_jual_produk`.`dproduk_diskon_jenis`, '-'))
             AS `diskon_jenis`,
          if(ifnull(`karyawan`.`karyawan_username`, '-') = '',
             '-',
             ifnull(`karyawan`.`karyawan_username`, '-'))
             AS `sales`,
          ...
          if(
             (`master_jual_produk`.`jproduk_stat_dok` = 'Batal'),
             0,
...
AS
   SELECT `detail_jual_rawat`.`drawat_master` AS `drawat_master`,
          ...
          if(ifnull(`detail_jual_rawat`.`drawat_diskon_jenis`, '-') = '',
             '-',
             ifnull(`detail_jual_rawat`.`drawat_diskon_jenis`, '-'))
             AS `diskon_jenis`,
          if(
          ...
          `vu_perawatan`.`rawat_id` AS `produk_id`
...
AS
   SELECT `detail_jual_paket`.`dpaket_master` AS `dpaket_master`,
          ...
          if(ifnull(`detail_jual_paket`.`dpaket_diskon_jenis`, '-') = '',
             '-',
             ifnull(`detail_jual_paket`.`dpaket_diskon_jenis`, '-'))
             AS `diskon_jenis`,
          if(ifnull(`detail_jual_paket`.`dpaket_sales`, '-') = '',
             '-',
             ifnull(`detail_jual_paket`.`dpaket_sales`, '-'))
             AS `sales`,
