...
/* 	These code was generated using phpCIGen v 0.1.b (21/04/2009)
	...
	+ Filename 		: C_appointment_nonmedis.php
 	+ Author  		: masongbee
...
class C_appointment_nonmedis extends Controller {

	//constructor
	function C_appointment_nonmedis(){
		parent::Controller();
		$this->load->model('m_appointment_nonmedis', '', TRUE);
	}
	...
	function index(){
		...
		$this->load->view('main/v_appointment_nonmedis');
	}
	...
	function  detail_appointment_detail_list(){
		...
		$result=$this->m_appointment_nonmedis->detail_appointment_detail_list($master_id,$query,$start,$end);
		echo $result;
	}
	//end of handler
	...
	function detail_appointment_detail_purge(){
		$master_id = (integer) (isset($_POST['master_id']) ? $_POST['master_id'] : $_GET['master_id']);
		$result=$this->m_appointment_nonmedis->detail_appointment_detail_purge($master_id);
	}
	//eof
	
	//get master id, note: not done yet
	function get_master_id(){
		$result=$this->m_appointment_nonmedis->get_master_id();
		echo $result;
	}
	//
	...
	function appointment_list(){
		...
		$start = (integer) (isset($_POST['start']) ? $_POST['start'] : $_GET['start']);
		$end = (integer) (isset($_POST['limit']) ? $_POST['limit'] : $_GET['limit']);
		$result=$this->m_appointment_nonmedis->appointment_list($query,$start,$end);
		echo $result;
	}
	...
	function appointment_delete(){
		...
		$result=$this->m_appointment_nonmedis->appointment_delete($pkid);
		echo $result;
	}
	...
	function appointment_search(){
		...
		$start = (integer) (isset($_POST['start']) ? $_POST['start'] : $_GET['start']);
		$end = (integer) (isset($_POST['limit']) ? $_POST['limit'] : $_GET['limit']);
		$result = $this->m_appointment_nonmedis->appointment_search($app_id ,$app_customer ,$app_tanggal ,$app_cara ,$app_keterangan ,$start,$end);
		echo $result;
	}
	...
	function appointment_print(){
		...
		$result = $this->m_appointment_nonmedis->appointment_print($app_id ,$app_customer ,$app_tanggal ,$app_cara ,$app_keterangan ,$option,$filter);
	...
	function appointment_export_excel(){
		...
		$query = $this->m_appointment_nonmedis->appointment_export_excel($app_id ,$app_customer ,$app_tanggal ,$app_cara ,$app_keterangan ,$option,$filter);
