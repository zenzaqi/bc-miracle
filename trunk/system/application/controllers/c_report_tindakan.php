...
class C_report_tindakan extends Controller {
	...
	function tindakan_search(){
		...
		$start = (integer) (isset($_POST['start']) ? $_POST['start'] : $_GET['start']);
		$end = (integer) (isset($_POST['limit']) ? $_POST['limit'] : $_GET['limit']);
		$result = $this->m_report_tindakan->tindakan_search($trawat_id ,$trawat_tglapp_start ,$trawat_tglapp_end ,$trawat_dokter ,$start,$end);
		echo $result;
	}
	...
	function tindakan_export_excel(){
		...
		$query = $this->m_report_tindakan->tindakan_export_excel($trawat_id ,$trawat_dokter ,$option,$filter);
