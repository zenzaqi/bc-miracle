...
class M_main extends Model{
	...
	function get_menus(){
		if($_SESSION["userid"]==='Super Admin')
			...
			$sql="select * from vu_menus where group_id='".$_SESSION["usergroup"]."' and menu_parent=0 order by menu_position asc ";
	...
	function get_sub_menus(){
		if($_SESSION["userid"]==='Super Admin')
			...
			$sql="select * from vu_menus where group_id='".$_SESSION["usergroup"]."' and menu_parent<>0 order by menu_parent,menu_position asc ";
		$query=$this->db->query($sql);
		$rs=$query->result_array();
		return $rs;
	}
	...
	function get_shortcuts(){
		if($_SESSION["userid"]==='Super Admin')
			...
			$sql="select * from vu_menus where menu_parent<>0 and group_id='".$_SESSION["usergroup"]."' and menu_leftpanel='Y' order by menu_parent,menu_position asc ";
		$query=$this->db->query($sql);
		$rs=$query->result_array();
		return $rs;
	}
