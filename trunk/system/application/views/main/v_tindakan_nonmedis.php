...
Ext.onReady(function(){
	...
	function tindakan_nonmedis_update(oGrid_event){
		...
		var trawat_jamdatang_update=null;
//		var trawat_appointment_update=null;
		...
		if(oGrid_event.record.data.trawat_jamdatang!== null){trawat_jamdatang_update = oGrid_event.record.data.trawat_jamdatang;}
//		if(oGrid_event.record.data.trawat_appointment!== null){trawat_appointment_update = oGrid_event.record.data.trawat_appointment;}
		...
		Ext.Ajax.request({  
			...
			params: {
				...
				trawat_jamdatang	:trawat_jamdatang_update,  
//				trawat_appointment	:trawat_appointment_update,  
...
//		trawat_nonmedis_appointmentField.setValue(tindakan_nonmedisListEditorGrid.getSelectionModel().getSelected().get('trawat_appointment'));
	...
	tindakan_nonmedis_DataStore = new Ext.data.Store({
		...
		/* dataIndex => insert intotindakan_nonmedis_ColumnModel, Mapping => for initiate table column */ 
			...
			{name: 'trawat_jamdatang', type: 'string', mapping: 'trawat_jamdatang'}, 
			{name: 'trawat_appointment', type: 'string', mapping: 'trawat_appointment'}, 
...
//		}, 
	...
	trawat_nonmedis_custField= new Ext.form.TextField({
	...
	/* Identify  trawat_jamdatang Field */
	trawat_nonmedis_jamdatangField= new Ext.form.TextField({
		id: 'trawat_nonmedis_jamdatangField',
...
//	});
		...
		items:[
			{
				...
				layout: 'form',
				border:false,
				items: [trawat_nonmedis_custField, trawat_nonmedis_jamdatangField, trawat_nonmedis_idField] 
			},
			{
				columnWidth:0.5,
				layout: 'form',
				border:false,
				items: [trawat_nonmedis_keteranganField] 
			}
			]
	...
	cbo_dtrawat_petugas_nonmedisDataSore = new Ext.data.Store({
		...
		proxy: new Ext.data.HttpProxy({
			url: 'index.php?c=c_tindakan_nonmedis&m=get_karyawan_nonmedis_list', 
			method: 'POST'
		}),
		reader: new Ext.data.JsonReader({
			root: 'results',
	...
	var combo_dtrawat_perawatan=new Ext.form.ComboBox({
			store: cbo_dtrawat_perawatan_nonmedisDataSore,
			mode: 'remote',
			typeAhead: true,
			displayField: 'dtrawat_perawatan_display',
			valueField: 'dtrawat_perawatan_value',
			triggerAction: 'all',
			lazyRender:true,

	});
	
	var combo_dtrawat_petugas=new Ext.form.ComboBox({
	...
	var combo_dtrawat_petugas2=new Ext.form.ComboBox({
			store: cbo_dtrawat_petugas_nonmedisDataSore,
			mode: 'remote',
			typeAhead: true,
			displayField: 'dtrawat_karyawan_display',
	...
	function tindakan_nonmedis_detail_insert(){
		for(i=0;i<tindakan_nonmedis_detail_DataStore.getCount();i++){
			...
			Ext.Ajax.request({
				waitMsg: 'Please wait...',
				url: 'index.php?c=c_tindakan_nonmedis&m=detail_tindakan_detail_insert',
				params:{
	...
	function tindakan_nonmedis_detail_purge(){
		Ext.Ajax.request({
			waitMsg: 'Please wait...',
			url: 'index.php?c=c_tindakan_nonmedis&m=detail_tindakan_detail_purge',
