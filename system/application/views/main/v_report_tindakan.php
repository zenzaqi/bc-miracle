...
Ext.onReady(function(){
	...
	cbo_tmedis_cutomerDataStore = new Ext.data.Store({
		...
		proxy: new Ext.data.HttpProxy({
			url: 'index.php?c=c_report_tindakan&m=get_customer_list', 
			method: 'POST'
		}),
	...
	tindakanListEditorGrid =  new Ext.grid.EditorGridPanel({
		...
		/*{
			...
			listeners:{
				...
				Ext.get(this.id).set({qtip:'- Nama Dokter<br>- Status'});
				}
			},
			width: 120
		}),'-',{
	...
	cbo_dtindakan_terapisDataStore = new Ext.data.Store({
		...
		proxy: new Ext.data.HttpProxy({
			url: 'index.php?c=c_report_tindakan&m=get_terapis_list', 
			method: 'POST'
		}),baseParams: {start: 0, limit: 15 },
		reader: new Ext.data.JsonReader({
			root: 'results',
	...
	function tindakan_medisdetail_insert(){
		for(i=0;i<tindakan_medis_detail_DataStore.getCount();i++){
			...
			if(tindakan_medisdetail_record.data.dtrawat_perawatan!=""){
				Ext.Ajax.request({
					waitMsg: 'Please wait...',
					url: 'index.php?c=c_report_tindakan&m=detail_tindakan_medis_detail_insert',
					params:{
	...
	function tindakan_medisdetail_purge(){
		Ext.Ajax.request({
			waitMsg: 'Please wait...',
			url: 'index.php?c=c_report_tindakan&m=detail_tindakan_medis_detail_purge',
	...
	cbo_perawatan_dtjnonmedisDataStore = new Ext.data.Store({
		...
		proxy: new Ext.data.HttpProxy({
			url: 'index.php?c=c_report_tindakan&m=get_nonmedis_in_tmedis_list', 
			method: 'POST'
		}),baseParams: {start: 0, limit: 15 },
			reader: new Ext.data.JsonReader({
			root: 'results',
	...
	function dtindakan_jual_nonmedis_insert(){
		if(dtindakan_jual_nonmedisDataStore.getCount()!=0){
			for(i=0;i<dtindakan_jual_nonmedisDataStore.getCount();i++){
				...
				Ext.Ajax.request({
					waitMsg: 'Please wait...',
					url: 'index.php?c=c_report_tindakan&m=detail_dtindakan_jual_nonmedis_insert',
					params:{
	...
	function dtindakan_jual_nonmedis_purge(){
		Ext.Ajax.request({
			waitMsg: 'Please wait...',
			url: 'index.php?c=c_report_tindakan&m=detail_tindakan_nonmedis_detail_purge',
			params:{ master_id: eval(trawat_medis_idField.getValue()) },
			callback: function(opts, success, response){
				if(success){
					//dtindakan_jual_nonmedis_insert();
	...
	function tindakan_medisexport_excel(){
		...
		//if(tindakan_medisDataStore.baseParams.rawat_nama!==null){tindakan_perawatan_2excel = tindakan_medisDataStore.baseParams.rawat_nama;}
		...
		params: {
			...
			//rawat_nama : tindakan_perawatan_2excel,
		  	currentlisting: tindakan_medisDataStore.baseParams.task // this tells us if we are searching or not
		},
		success: function(response){              
		  	var result=eval(response.responseText);
