...
class M_report_tukar_point extends Model{
		
		//constructor
		function M_report_tukar_point() {
			parent::Model();
		}
		
		function printreport_tukar_point($tukar_point_cust,$tukar_point_tglstart,$tukar_point_tglend){
			//full query
...
//			}
			...
			if($tukar_point_cust!=''){
				...
				$query.= " epoint_cust = '".$tukar_point_cust."'";
			};
			if($tukar_point_tglstart!=''){
				$query.=eregi("WHERE",$query)?" AND ":" WHERE ";
				$query.= " (epoint_tanggal between '".$tukar_point_tglstart."' and '".$tukar_point_tglend."')";
			};
